  ...
  return (
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 1 }}>max elevation</div>
      </div>
      <div>
        <div style={{ fontSize: 14, fontWeight: 500 }}>{durStr}</div>
        <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 1 }}>duration</div>
      </div>
...
function MetricCard({ label, value, sub }) {
  return (
    <div style={{ background: "var(--color-background-secondary)", borderRadius: 8, padding: "0.85rem 1rem" }}>
...
export default function App() {
  ...
  const fetchForecast = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/passes/norad/${noradId}?lat=${lat}&lon=${lon}&hours_ahead=48`);
  ...
  return (
    <div style={{ padding: "1.25rem", fontFamily: "var(--font-sans)", maxWidth: 900, margin: "0 auto" }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        borderBottom: "0.5px solid var(--color-border-tertiary)", paddingBottom: "0.75rem", marginBottom: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 18, fontWeight: 500 }}>
          ...
          OrbitGuard
          <span style={{ fontSize: 12, color: "var(--color-text-secondary)", fontWeight: 400 }}>pass quality forecaster</span>
        </div>
        {wx && (
      ...
      {wx && (
        <div style={{ display: "flex", gap: 10, alignItems: "center", padding: "0.75rem 1rem",
          background: "var(--color-background-secondary)", borderRadius: 8, marginBottom: "1rem" }}>
          {[["Kp index", wx.kp_index.toFixed(1)], ["F10.7 flux", wx.f107_solar_flux.toFixed(0)],
            ["Active alerts", wx.active_alert_count], ["Condition", wx.severity]].map(([lbl, val], i) => (
            <div key={lbl} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
              {i > 0 && <div style={{ position: "absolute", width: "0.5px", height: 32, background: "var(--color-border-tertiary)" }} />}
              <span style={{ fontSize: 16, fontWeight: 500 }}>{val}</span>
              <span style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 2 }}>{lbl}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 10, marginBottom: "1rem", alignItems: "end" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <label style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>NORAD ID</label>
          <select value={noradId} onChange={e => setNoradId(Number(e.target.value))} style={{ fontSize: 14 }}>
            {POPULAR_SATS.map(s => <option key={s.norad_id} value={s.norad_id}>{s.name} ({s.norad_id})</option>)}
          </select>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <label style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Latitude</label>
          <input type="number" step="0.01" value={lat} onChange={e => setLat(Number(e.target.value))} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <label style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Longitude</label>
      ...
      {error && <div style={{ background: "#FCEBEB", color: "#501313", borderRadius: 8, padding: "0.75rem 1rem", fontSize: 13, marginBottom: "1rem" }}>{error}</div>}

      {/* Chart */}
      {chartData.length > 0 && (
        <div style={{ marginBottom: "1rem" }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: "var(--color-text-secondary)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.6rem" }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "2.5rem", color: "var(--color-text-secondary)", fontSize: 14 }}>
          No passes found. Try adjusting the ground station coordinates or satellite.
        </div>
      ) : (
