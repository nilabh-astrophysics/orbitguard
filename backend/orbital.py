...
Includes hardcoded fallback TLEs for popular satellites.
"""
...
# Fallback TLEs (updated May 2025) for when CelesTrak is unreachable
FALLBACK_TLES = {
    25544: ("ISS (ZARYA)",
...
def get_tle(norad_id: int) -> Optional[tuple]:
    """Try CelesTrak first, fall back to hardcoded TLEs."""
    tle = fetch_tle_from_celestrak(norad_id)
    if tle:
        return tle
    if norad_id in FALLBACK_TLES:
        logger.info(f"Using fallback TLE for NORAD {norad_id}")
...
def passes_from_norad(
    ...
    if tle is None:
        return {"error": f"Could not fetch TLE for NORAD ID {norad_id} and no fallback available.", "norad_id": norad_id}
